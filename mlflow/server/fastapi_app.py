...
from mlflow.tracking._workspace.context import reset_workspace, set_current_workspace
from mlflow.version import VERSION

FASTAPI_NATIVE_PREFIXES = ("/ajax-api/3.0", OTLP_TRACES_PATH)
...
def add_fastapi_workspace_middleware(fastapi_app: FastAPI) -> None:
    ...
    async def workspace_context_middleware(request: Request, call_next):
        ...
        resource_type = _resolve_rbac_resource_type(path)
        token = set_current_workspace(workspace.name)
        request.state.mlflow_workspace = workspace
        request.state.mlflow_rbac_resource_type = resource_type

        try:
            response = await call_next(request)
        finally:
            reset_workspace(token)
            if hasattr(request.state, "mlflow_workspace"):
                delattr(request.state, "mlflow_workspace")
            if hasattr(request.state, "mlflow_rbac_resource_type"):
                delattr(request.state, "mlflow_rbac_resource_type")
        return response
