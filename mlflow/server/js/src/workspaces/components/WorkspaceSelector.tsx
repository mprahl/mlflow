...
export const WorkspaceSelector = () => {
  ...
  // Redirect to fallback workspace if current workspace is no longer available
  useEffect(() => {
    if (!isLoading && !isError && workspaces.length > 0 && currentWorkspace) {
      const workspaceExists = workspaces.some((ws) => ws.name === currentWorkspace);
      if (!workspaceExists) {
