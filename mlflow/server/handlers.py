...
def initialize_backend_stores(
    ...
    resolved_artifact_root = default_artifact_root or os.environ.get(ARTIFACT_ROOT_ENV_VAR, None)

    if _workspaces_enabled_flag() and not resolved_artifact_root:
        raise MlflowException.invalid_parameter_value(
            "--enable-workspaces requires --default-artifact-root or the "
            "MLFLOW_ARTIFACT_ROOT environment variable to be set."
        )
...
def _workspace_contains_resources(workspace_name: str) -> bool:
    ...
    if tracking_store is not None:
        with workspace_context.WorkspaceContext(workspace_name):
            experiments = list(
                tracking_store.search_experiments(view_type=ViewType.ALL, max_results=2)
            )
        if any(exp.name != Experiment.DEFAULT_EXPERIMENT_NAME for exp in experiments):
            return True
        default_experiment = next(
            (exp for exp in experiments if exp.name == Experiment.DEFAULT_EXPERIMENT_NAME),
            None,
        )
        ...
        if default_experiment is not None:
            ...
            if len(runs_page) > 0:
                # The implicit default experiment still holds runs; consider the workspace non-empty
                # even if users never created named experiments.
                return True
...
def _add_workspace_prefix(base_path: str) -> str | None:
    ...
    if base_path.startswith(artifacts_prefix):
        suffix = base_path[len(artifacts_prefix) :].lstrip("/")
        return (
            f"/mlflow-artifacts/workspaces/<workspace_name>/{suffix}"
            if suffix
            else ("/mlflow-artifacts/workspaces/<workspace_name>")
        )
...
def _insert_workspace_segment(path: str) -> str:
    ...
    if artifacts_segment in path and workspace_segment not in path:
        return path.replace(
            artifacts_segment,
            f"{artifacts_segment}{workspace_segment}",
            1,
        )
