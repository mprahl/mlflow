...
class AuthContext:
    ...
    @classmethod
    def from_authorization(cls, auth: Authorization) -> "AuthContext":
        return cls(
...
def _get_workspace_store_optional():
    ...
    try:
        ...
        if e.error_code == ErrorCode.Name(FEATURE_DISABLED):
            # Expected when workspaces are not enabled
            _logger.debug("Workspace store not available: workspaces not enabled")
            return None
        raise
...
def _workspace_permission(
    ...
    auth_ctx = AuthContext.from_authorization(auth)

    if auth_ctx.is_admin:
        return MANAGE

    if auth_ctx.username is None:
...
def _has_resource_read_access(
    ...
    # In practice, this means workspaces are not enabled.
...
def _get_permission_from_experiment_id() -> Permission:
    ...
    return _get_permission_from_store_or_default(
        lambda: store.get_experiment_permission(experiment_id, auth_ctx.username).permission,
...
def _get_permission_from_experiment_name() -> Permission:
    ...
    return _get_permission_from_store_or_default(
        lambda: store.get_experiment_permission(
            store_exp.experiment_id, auth_ctx.username
        ).permission,
...
def _get_permission_from_registered_model_name() -> Permission:
    ...
    auth_ctx = AuthContext.from_authorization(auth)
    return _get_permission_from_store_or_default(
        lambda: store.get_registered_model_permission(name, auth_ctx.username).permission,
...
def validate_can_create_experiment() -> bool:
    # In single tenant mode, everyone can create experiments.
    if not _workspaces_enabled_flag():
        return True
...
def validate_can_create_registered_model() -> bool:
    # In single tenant mode, everyone can create registered models.
    if not _workspaces_enabled_flag():
        return True
...
def validate_can_view_workspace() -> bool:
    ...
    names = set(store.list_accessible_workspace_names(auth_ctx.username))
...
for path, method, validator in _FLASK_ROUTE_VALIDATORS:
    BEFORE_REQUEST_VALIDATORS[(path, method)] = validator
    workspace_path = get_workspace_prefixed_path(path)
    if workspace_path != path:
        BEFORE_REQUEST_VALIDATORS[(workspace_path, method)] = validator
...
def list_workspace_permissions(workspace_name: str):
    ...
    workspace_store.get_workspace(workspace_name, request)
...
def filter_list_workspaces(resp: Response) -> None:
    ...
    if hasattr(store, "supports_workspaces") and store.supports_workspaces():
        if auth_ctx.username is not None:
            allowed = set(store.list_accessible_workspace_names(auth_ctx.username))
            if auth_config.grant_default_workspace_access:
...
def _cleanup_workspace_permissions(resp: Response) -> None:
    # This handler runs only on successful DELETE responses.
    workspace_name = request.view_args.get("workspace_name") if request.view_args else None
    if not workspace_name:
        return
...
def filter_search_experiments(resp: Response):
    ...
    auth = authenticate_request()
    auth_ctx = AuthContext.from_authorization(auth)
    perms = store.list_experiment_permissions(auth_ctx.username)
    can_read = {p.experiment_id: get_permission(p.permission).can_read for p in perms}
    default_can_read = get_permission(auth_config.default_permission).can_read
...
def filter_search_registered_models(resp: Response):
    ...
    auth = authenticate_request()
    auth_ctx = AuthContext.from_authorization(auth)
    perms = store.list_registered_model_permissions(auth_ctx.username)
...
_WORKSPACE_AFTER_REQUEST_ENTRIES = [
    ("/api/2.0/mlflow/workspaces", "GET", filter_list_workspaces, True),
    ("/ajax-api/2.0/mlflow/workspaces", "GET", filter_list_workspaces, True),
    (
...
for path, method, handler, include_workspace_variant in _WORKSPACE_AFTER_REQUEST_ENTRIES:
    for candidate in {path, _add_static_prefix(path)}:
        AFTER_REQUEST_HANDLERS[(candidate, method)] = handler

    if include_workspace_variant:
        workspace_path = get_workspace_prefixed_path(path)
        for candidate in {workspace_path, _add_static_prefix(workspace_path)}:
            AFTER_REQUEST_HANDLERS[(candidate, method)] = handler
