...
class MlflowClient:
    ...
    def get_workspace_uri(self) -> str | None:
        """
        Return the resolved workspace provider URI (or None if not configured).
        """
    ...
    def _get_workspace_client(self) -> WorkspaceProviderClient:
        ...
        if self._workspace_uri is None:
            raise MlflowException(
                "Workspace URI resolution failed. Set the MLFLOW_WORKSPACE_URI environment "
                "variable to the desired workspace URI, or pass `workspace_uri` to MlflowClient.",
