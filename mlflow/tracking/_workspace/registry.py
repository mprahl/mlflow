...
_building_workspace_store_lock = threading.Lock()
...
class WorkspaceStoreRegistry(StoreRegistry):
    ...
    def get_store(self, workspace_uri: str, tracking_uri: str):
        """Return a workspace store instance for the provided URI."""

        return self._get_store_with_resolved_uri(workspace_uri, tracking_uri)

    @lru_cache(maxsize=100)
    def _get_store_with_resolved_uri(self, workspace_uri: str, tracking_uri: str):
        with _building_workspace_store_lock:
            ...
            return builder(workspace_uri=workspace_uri, tracking_uri=tracking_uri)
...
def _get_sqlalchemy_workspace_store(workspace_uri: str, tracking_uri: str):
    from mlflow.store.workspace.sqlalchemy_store import SQLAlchemyStore

    return SQLAlchemyStore(workspace_uri)


def _get_rest_workspace_store(workspace_uri: str, tracking_uri: str):
...
def get_workspace_store(workspace_uri: str, tracking_uri: str):
    """
    Return a workspace store for the specified workspace URI.

    Args:
        workspace_uri: Workspace backend URI.
        tracking_uri: Tracking store URI associated with the workspace.
    """

    return _get_workspace_store_registry().get_store(
        workspace_uri=workspace_uri, tracking_uri=tracking_uri
    )
