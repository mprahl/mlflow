...
class SqlTraceInfo(Base):
    ...
    """
    input_tokens = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized token usage metric for trace analytics queries.
    """
    output_tokens = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized token usage metric for trace analytics queries.
    """
    total_tokens = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized token usage metric for trace analytics queries.
    """
    cache_read_input_tokens = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized cache-read token usage metric for trace analytics queries.
    """
    cache_creation_input_tokens = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized cache-creation token usage metric for trace analytics queries.
    """
    input_cost = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized trace input cost metric for trace analytics queries.
    """
    output_cost = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized trace output cost metric for trace analytics queries.
    """
    total_cost = Column(sa.types.Float(precision=53), nullable=True)
    """
    Denormalized trace total cost metric for trace analytics queries.
    """
    ...
    def to_mlflow_entity(self):
        ...
        tags.pop(TraceTagKey.TRACE_NAME, None)
        if self.trace_name is not None:
            tags[TraceTagKey.TRACE_NAME] = self.trace_name

        trace_metadata = {metadata.key: metadata.value for metadata in self.request_metadata}
        trace_metadata.pop(TraceMetadataKey.TRACE_SESSION, None)
        trace_metadata.pop(TraceMetadataKey.TOKEN_USAGE, None)
        trace_metadata.pop(TraceMetadataKey.COST, None)
        if self.session_id is not None:
            trace_metadata[TraceMetadataKey.TRACE_SESSION] = self.session_id

        token_usage = {
            TokenUsageKey.INPUT_TOKENS: self.input_tokens,
            TokenUsageKey.OUTPUT_TOKENS: self.output_tokens,
            TokenUsageKey.TOTAL_TOKENS: self.total_tokens,
            TokenUsageKey.CACHE_READ_INPUT_TOKENS: self.cache_read_input_tokens,
            TokenUsageKey.CACHE_CREATION_INPUT_TOKENS: self.cache_creation_input_tokens,
        }
        if any(value is not None for value in token_usage.values()):
            trace_metadata[TraceMetadataKey.TOKEN_USAGE] = json.dumps({
                key: value for key, value in token_usage.items() if value is not None
            })

        cost = {
            CostKey.INPUT_COST: self.input_cost,
            CostKey.OUTPUT_COST: self.output_cost,
            CostKey.TOTAL_COST: self.total_cost,
        }
        if any(value is not None for value in cost.values()):
            trace_metadata[TraceMetadataKey.COST] = json.dumps({
...
class SqlAssessments(Base):
    ...
    """
    Denormalized experiment ID for analytics queries.
    """
    trace_timestamp_ms = Column(BigInteger, nullable=True)
    """
    Denormalized trace request timestamp used for assessment analytics filters.
    """
    aggregate_value = Column(sa.types.Float(precision=53), nullable=True)
    """
    Materialized numeric assessment value used for AVG and percentile queries.
    """
...
class SqlSpan(Base):
    ...
    experiment_id = Column(Integer, nullable=False)
    """
    Experiment ID: `Integer`. Validated at the application layer from the owning trace.
    """
    ...
    __table_args__ = (
        ...
        Index(
            ...
            postgresql_where=(
                input_cost.isnot(None) | output_cost.isnot(None) | total_cost.isnot(None)
            ),
        ),
        Index(
