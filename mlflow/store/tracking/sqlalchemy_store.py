...
class SqlAlchemyStore(AbstractStore):
    ...
    def __init__(self, db_uri, default_artifact_root):
        ...
        if self._workspaces_enabled():
            can_get_default = True
            current_workspace = workspace_context.get_current_workspace()
            try:
                provider = self._get_workspace_provider_instance()
                default_workspace = provider.get_default_workspace(None)
                workspace_context.set_current_workspace(default_workspace.name)
            except NotImplementedError:
                can_get_default = False

            try:
                if (
                    can_get_default
                    and self.get_experiment_by_name(Experiment.DEFAULT_EXPERIMENT_NAME) is None
                ):
                    with self.ManagedSessionMaker() as session:
                        self._create_default_experiment(session)
            finally:
                workspace_context.set_current_workspace(current_workspace)
        else:
            try:
        ...
        with self.ManagedSessionMaker() as session:
            try:
                ...
                if not artifact_location:
                    ...
                    eid = eid_query.first().experiment_id
