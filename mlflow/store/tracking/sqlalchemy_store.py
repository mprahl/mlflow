...
class SqlAlchemyStore(AbstractStore):
    ...
    def _get_workspace_provider_instance(self):
        if self._workspace_provider is None:
            self._workspace_provider = get_workspace_store(
                workspace_uri=self.db_uri,
                tracking_uri=self.db_uri,
            )
    ...
    def _ensure_default_workspace_experiment(self) -> None:
        ...
        try:
            provider = self._get_workspace_provider_instance()
            default_workspace = provider.get_default_workspace(None)
        ...
        with self.ManagedSessionMaker() as session:
            try:
                ...
                if not artifact_location:
                    ...
                    eid_query = session.query(SqlExperiment).filter_by(
                        name=name,
                        workspace=workspace,
                    )
                    eid_result = eid_query.first()
                    if eid_result is None:
                        raise MlflowException(
                            f"Failed to retrieve experiment ID after creation for '{name}'",
                            INTERNAL_ERROR,
                        )
                    eid = eid_result.experiment_id
                    experiment.artifact_location = self._get_artifact_location(eid, workspace)
                session.flush()
    ...
    def _log_inputs_impl(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            existing_datasets = existing_datasets_query.all()
            dataset_uuids = {}
    ...
    def log_spans(self, location: str, spans: list[Span], tracking_uri=None) -> list[Span]:
        ...
        with self.ManagedSessionMaker() as session:
            ...
            update_filters = [SqlTraceInfo.request_id == trace_id]
            # Keep the update scoped to the trace's experiment so we can't mutate traces
            # from another workspace when multi-tenancy is enabled.
            update_filters.append(SqlTraceInfo.experiment_id == sql_trace_info.experiment_id)

            session.query(SqlTraceInfo).filter(*update_filters).update(
                update_dict,
            ...
            # Filter by experiment_id to ensure we can't mutate traces from another workspace
    ...
    def create_dataset(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            resolved_experiment_ids: list[str] = []
            dataset_workspace = workspace

            if self._workspaces_enabled() and not experiment_ids:
                raise MlflowException.invalid_parameter_value(
                    "Creating an evaluation dataset requires at least one experiment when "
                    "workspaces are enabled."
                )

            for exp_id in experiment_ids:
                experiment = self._get_experiment(session, str(exp_id), ViewType.ALL)
                if self._workspaces_enabled() and experiment.workspace != workspace:
                    raise MlflowException.invalid_parameter_value(
                        "Evaluation datasets can only include experiments from the active "
                        "workspace."
                    )

                resolved_experiment_ids.append(str(experiment.experiment_id))

            experiment_ids = list(dict.fromkeys(resolved_experiment_ids))
    ...
    def search_datasets(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            workspace_filter = self._dataset_workspace_filter()
            if workspace_filter is not None:
                stmt = stmt.filter(workspace_filter)
    ...
    def upsert_dataset_records(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            session.query(SqlEvaluationDataset).filter(
                ...
                SqlEvaluationDataset.workspace == self._get_active_workspace(),
    ...
    def add_dataset_to_experiments(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            validated_exp_ids: list[str] = []
            for exp_id in experiment_ids:
                # Check if experiment exists in the workspace
                experiment = (
                    session.query(SqlExperiment)
                    .filter(
                        SqlExperiment.experiment_id == str(exp_id),
                        SqlExperiment.workspace == dataset_workspace,
                    )
                    .first()
                )
                if not experiment:
                ...
                validated_exp_ids.append(str(experiment.experiment_id))

            validated_exp_ids = list(dict.fromkeys(validated_exp_ids))
            ...
            existing_associations = (
                ...
                .filter(
                    ...
                    SqlEntityAssociation.destination_id.in_(validated_exp_ids),
            ...
            new_associations = []
            for exp_id in validated_exp_ids:
        ...
        with self.ManagedSessionMaker() as session:
            ...
            for exp_id in experiment_ids:
                ...
                experiment = (
                    ...
                    .filter(
                        SqlExperiment.experiment_id == str(exp_id),
                        SqlExperiment.workspace == dataset.workspace,
                    )
                    .first()
                )
                if not experiment:
    ...
    def _runs_query(self, session):
        ...
        return (
            ...
            .join(SqlExperiment, SqlExperiment.experiment_id == SqlRun.experiment_id)
            .filter(SqlExperiment.workspace == workspace)
        )
    ...
    def _logged_model_query(self, session):
        ...
        return session.query(SqlLoggedModel).filter(
            SqlLoggedModel.experiment_id.in_(select(workspace_experiment_ids))
        )
    ...
    def _dataset_query(self, session):
        ...
        return session.query(SqlEvaluationDataset).filter(
            SqlEvaluationDataset.workspace == self._get_active_workspace()
        )
