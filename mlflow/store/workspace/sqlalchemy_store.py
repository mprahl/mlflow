...
class SQLAlchemyStore(AbstractStore):
    ...
    def list_workspaces(self, request) -> Iterable[Workspace]:
        with self.ManagedSessionMaker() as session:
            rows = session.query(SqlWorkspace).all()
            return [Workspace(name=row.name, description=row.description) for row in rows]

    def get_workspace(self, workspace_name: str, request) -> Workspace:
        with self.ManagedSessionMaker() as session:
            workspace = self._get_workspace(session, workspace_name)
            return Workspace(name=workspace.name, description=workspace.description)
    ...
    def update_workspace(self, workspace: Workspace, request) -> Workspace:
        with self.ManagedSessionMaker() as session:
            ...
            return Workspace(name=entity.name, description=entity.description)

    def delete_workspace(self, workspace_name: str, request) -> None:
        if workspace_name == DEFAULT_WORKSPACE_NAME:
            raise MlflowException.invalid_state(
    ...
    def get_default_workspace(self, request) -> Workspace:
        return self.get_workspace(DEFAULT_WORKSPACE_NAME, request)
    ...
    def _get_auth_store():
        try:
            ...
            return None
        return getattr(auth_module, "store", None)
