...
def test_workspace_scoped_repo_path_prefixes_workspace(monkeypatch):
    monkeypatch.setenv(MLFLOW_ENABLE_WORKSPACES.name, "true")

    with workspace_context.WorkspaceContext("team-a"):
        assert handlers._workspace_scoped_repo_path("5/abc") == "workspaces/team-a/5/abc"
        assert handlers._workspace_scoped_repo_path(None) == "workspaces/team-a"
        assert (
            handlers._workspace_scoped_repo_path("workspaces/team-a/5/abc")
            == "workspaces/team-a/5/abc"
        )

    workspace_context.clear_workspace()
    assert handlers._workspace_scoped_repo_path("foo/bar") == "foo/bar"
...
def test_workspace_scoped_repo_path_blocks_cross_workspace_access(monkeypatch):
    ...
    with workspace_context.WorkspaceContext("team-a"):
        with pytest.raises(MlflowException, match="team-b"):
            handlers._workspace_scoped_repo_path("workspaces/team-b/5/abc")

        with pytest.raises(MlflowException, match="include a workspace name"):
            handlers._workspace_scoped_repo_path("workspaces/")
