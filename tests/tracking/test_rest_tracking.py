...
from mlflow.server.auth.permissions import READ
...
def workspace_mlflow_client(
    ...
    try:
        init_store.create_workspace(
            WorkspaceEntity(name=DEFAULT_WORKSPACE_NAME, description="Default workspace"),
            request=None,
        )
...
def test_rest_tracking_respects_workspace_context(workspace_mlflow_client):
    ...
    with WorkspaceContext(team_b):
        ...
        assert all(run.info.experiment_id == exp_b for run in runs)
        assert run_a_id not in {run.info.run_id for run in runs}
    ...
    with WorkspaceContext(team_a):
        ...
        assert all(run.info.experiment_id == exp_a for run in runs)
        assert run_b_id not in {run.info.run_id for run in runs}
        run_ids = {run.info.run_id for run in client.search_runs([exp_a])}
        assert run_a_id in run_ids
...
def test_rest_run_operations_are_workspace_scoped(workspace_mlflow_client):
    ...
    with WorkspaceContext(team_b):
        ...
        assert all(run.info.experiment_id == exp_b for run in cross_runs)
...
def test_rest_artifact_operations_are_workspace_scoped(workspace_mlflow_client, tmp_path: Path):
    ...
    with WorkspaceContext(team_b):
        ...
        try:
            ...
            pytest.xfail("Artifact download across workspaces unexpectedly succeeded")
...
def _graphql_post(
    ...
    *,
    ...
    endpoint = "/graphql" if not workspace else f"/workspaces/{workspace}/graphql"
...
def test_workspace_permission_resource_type_detection(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(auth_module, "_get_workspace_store_optional", lambda: None)
    monkeypatch.setattr(auth_module, "_get_tracking_store", lambda: DummyTrackingStore())
    monkeypatch.setattr(auth_module, "_get_model_registry_store", lambda: DummyRegistryStore())

    def _record_permission(
        auth: Authorization,
        resource_type: str,
        workspace_name: str,
        grant_default_access: bool = True,
    ):
        recorded_calls.append((resource_type, workspace_name))
        return READ

    monkeypatch.setattr(auth_module, "_workspace_permission", _record_permission)

    auth = Authorization("basic", {"username": "alice", "password": "secret"})

    recorded_calls.clear()
    perm = auth_module._workspace_permission_for_experiment(auth, "exp-123")
    assert perm == READ
    assert recorded_calls == [("experiments", "team-experiments")]

    recorded_calls.clear()
    perm = auth_module._workspace_permission_for_registered_model(auth, "model-xyz")
    assert perm == READ
    assert recorded_calls == [("registered_models", "team-models")]
