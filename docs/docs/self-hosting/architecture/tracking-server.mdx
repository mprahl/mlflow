...
This serves both the Tracking UI and the REST API at `http://127.0.0.1:5000`, storing metadata in a
local SQLite database (`sqlite:///mlflow.db` by default). For backward compatibility, if a
`./mlruns` file store with existing experiment data is present, that store is reused instead. You
...
That covers local use. When you're ready to use a different backend, store artifacts remotely, or
expose the server to your team, see [Configure Server](#configure-server) below.

## MCP on the tracking server \{#mcp}

Expose genai MCP tools (experiments, runs, traces, registered scorers) over Streamable HTTP at
`/mcp` on the same uvicorn process as the tracking server. This is opt-in, uses the in-process
tracking store, and reuses the server's existing authentication.

```bash
pip install 'mlflow[mcp]'
mlflow server --enable-mcp
```

Or set `MLFLOW_SERVER_ENABLE_MCP=true`. HTTP MCP requires uvicorn (the default server) and is not
available with `--gunicorn-opts` or `--waitress-opts`. Clients authenticate with the same Basic /
token credentials as the REST API.

Models, deployments, and other local/exec tools are not served on `/mcp`. Enable those only on a
local stdio MCP process via `mlflow mcp run` and `MLFLOW_MCP_TOOLS`. See
[MLflow MCP Server](/genai/mcp).
...
MLflow APIs like <APILink fn="mlflow.start_run" />, <APILink fn="mlflow.log_param" />, <APILink fn="mlflow.start_trace" /> make API requests to your remote tracking server and log the data.
...
This section describes how to configure the tracking server for some common use cases. The section requires you to have a basic knowledge about the tracking server architecture, please visit [Architecture Overview](/self-hosting/architecture/overview) if you are not familiar with it yet.
...
| Isolate experiments, models, and prompts | `--enable-workspaces`                                   | [Getting Started with Workspaces](/self-hosting/workspaces/getting-started) |
...
By default, the tracking server uses SQLite database (`sqlite:///mlflow.db`) to store runs metadata.
You can configure a different backend store by adding the `--backend-store-uri` option:
...
MLflow can archive older trace span payloads from the tracking database to a configured artifact
location while keeping the traces readable in the UI and APIs.

::::note
**Requirements**

Trace archival requires a tracking store that supports server-owned archival. The archival
`location` must be a real artifact repository URI. The scheduler only archives finalized traces;
in-progress traces are skipped.
::::

Create a YAML config file and point `MLFLOW_TRACE_ARCHIVAL_CONFIG` at it:

```yaml title="trace-archival.yaml"
trace_archival:
  enabled: true
...
Then start the server with that configuration, either through the environment or with the
`--trace-archival-config` flag:
...
When workspaces are enabled, trace archival settings are resolved in the following order:

1. Global defaults provide the base archive location and retention period.
2. Workspace-level `trace_archival_config` overrides those defaults for all traces in the
   workspace.
3. Experiment-level settings can further reduce retention for a specific experiment.
...
If experiment `123` is instead configured for 180 days, its traces are retained for 90 days unless
`123` is added to `long_retention_allowlist`, in which case they are retained for 180 days.

Troubleshooting:

- If trace archival never runs, confirm that `MLFLOW_TRACE_ARCHIVAL_CONFIG` points to a valid YAML
  file and that `trace_archival.enabled` is `true`.
- Changes to `MLFLOW_TRACE_ARCHIVAL_CONFIG` are picked up without restarting MLflow. The server
  refreshes the config automatically with a short cache window (about 5 seconds) and applies
  updated settings on later scheduler checks. If a config refresh fails, MLflow continues using the
  last valid config until the file is fixed.
- If a workspace or experiment does not show an effective retention, check whether archival is
  disabled, misconfigured, or missing an archival location.
- If a longer experiment retention is ignored, add that experiment ID to
  `long_retention_allowlist` in the server config.

See [Archive Traces](/genai/tracing/observe-with-traces/archive-traces) for the end-user workflow
and archival failure / retry behavior, and [Getting Started with Workspaces](/self-hosting/workspaces/getting-started) for workspace-level examples.

### Server Jobs And Redis

MLflow server jobs use local SQLite-backed Huey queues by default. To use a shared Redis-backed
queue, set `MLFLOW_SERVER_JOB_HUEY_REDIS_URL` on every server instance.

Each job queue and the periodic-task queue use separate Huey namespaces, so they can share a Redis
database without consuming one another's tasks.

In a multi-replica deployment, enable periodic task scheduling on exactly one instance with
`MLFLOW_SERVER_JOB_ENABLE_PERIODIC_TASKS=true`; set it to `false` on the other replicas.

Periodic-task locks are flushed at startup for SQLite by default, but not for Redis because Redis
may be shared by live replicas. Operators can explicitly override this with
`MLFLOW_SERVER_JOB_FLUSH_PERIODIC_LOCKS_ON_STARTUP=true` only after confirming that no other
replica owns the locks.

### Remote artifacts store \{#tracking-server-artifact-store}

#### Using the Tracking Server for proxied artifact access (default)
...
:::note
If the MLflow server is started with `--no-serve-artifacts` option, the client directly pushes artifacts
...
see [Artifact Stores documentation](/self-hosting/architecture/artifact-store#artifacts-stores-manage-access).
:::
...
:::warning[important]
The server listens on `http://127.0.0.1:5000` by default and only accepts
...
to specify which domains can access your server. See [Security Configuration](/self-hosting/security/network)
for details.
:::
...
For detailed configuration options, see [Security Configuration](/self-hosting/security/network).
...
For notebook integration and UI embedding options, see [Network Security](/self-hosting/security/network) configuration.
