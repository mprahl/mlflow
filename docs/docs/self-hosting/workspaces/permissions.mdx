...
Workspace-scoped permissions provide convenient access control for all resources within a workspace. This feature works in conjunction with MLflow's existing resource-level permissions when [authentication](/self-hosting/security/basic-http-auth) is enabled.
...
# As alice
mlflow.set_auth("alice", "password")
workspaces = mlflow.list_workspaces()
print(workspaces)  # ["team-a"] - alice sees the workspace

# As bob
mlflow.set_auth("bob", "password")
workspaces = mlflow.list_workspaces()
print(workspaces)  # [] - bob doesn't see team-a
...
- [Workspace Providers](./workspace-providers) - Implement custom providers
