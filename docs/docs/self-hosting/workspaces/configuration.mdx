...
:::info Workspace-aware REST routes
When multi-tenancy is enabled, every REST endpoint must include the workspace segment:

- Standard tracking/model registry APIs: `/api/<version>/mlflow/workspaces/<workspace>/…`
- Artifact proxy APIs: `/api/<version>/mlflow-artifacts/workspaces/<workspace>/…`

UI requests (AJAX) follow the same pattern (`/ajax-api/<version>/…`). When calling the APIs manually, ensure the workspace segment is present; otherwise the server will reject the request before authorization runs.
:::
...
| `MLFLOW_WORKSPACE_URI`      | Override the workspace provider URI; falls back to the resolved tracking URI | Tracking URI          |
| `MLFLOW_WORKSPACE_PROVIDER` | Python class path for custom provider                                        | Built-in SQL provider |
