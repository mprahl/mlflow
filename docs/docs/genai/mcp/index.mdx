...
description: Connect AI assistants and coding tools to MLflow using the Model Context Protocol (MCP). Search traces, analyze performance, log feedback, and more from Claude Code, Codex, Cursor, and Gemini CLI.
---
...
## Tracking server HTTP (`/mcp`)

To expose genai MCP tools from the tracking server itself, start uvicorn with MCP enabled:

```bash
mlflow server --enable-mcp
```

Clients connect to `http://<host>:<port>/mcp` using Streamable HTTP and the same `Authorization`
header as the REST API. This path calls the tracking store in-process. It does **not** expose
models, deployments, `evaluate_traces`, or other local/exec tools, even if `MLFLOW_MCP_TOOLS` is
set on the server.

`--enable-mcp` requires the `mcp` extra and uvicorn. It cannot be combined with `--gunicorn-opts`
or `--waitress-opts`.

## Set up (local stdio)

Configure a **local** stdio MCP server in your MCP client by adding the server configuration to
your client's settings file. This is the way to opt into models, deployments, and other CLI-backed
tools (`MLFLOW_MCP_TOOLS=ml` or `all`) while running next to the IDE.
...
- `MLFLOW_MCP_TOOLS`: Controls which tool categories are enabled (see below)
...
**Supported values (stdio `mlflow mcp run` only):**
...
| `ml`              | Traditional ML workflow tools (stdio only)     | `experiments`, `runs`, `models`, `deployments` |
| `all`             | All available stdio tools                      | All of the above                               |
